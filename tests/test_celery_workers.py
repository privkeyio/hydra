...
def celery_worker():
    celery_app.conf.update(
        task_always_eager=True, result_backend="cache+memory://", broker_url="memory://"
    )
    yield
    celery_app.conf.update(
        task_always_eager=False, result_backend=None, broker_url=None
    )


def test_generate_code_success(celery_worker):
    with (
        patch("hydra.workers.tasks.get_config") as mock_config,
...
def test_priority_queue_routing():
    ...
    assert (
        celery_app.conf.task_routes["hydra.workers.tasks.generate_code_priority"][
            "queue"
        ]
        == "high_priority"
    )
