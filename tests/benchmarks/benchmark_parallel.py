...
class ParallelExecutionBenchmark:
        ...
        for worker_count in worker_counts:
            ...
            def resource_intensive():
                ...
                for i in range(task_count):
                    task_id = executor.submit_task(
                        f"resource_{i}",
                        lambda x=i: sum(range(x * 500, (x + 1) * 500))
                    )
                    task_ids.append(task_id)
    ...
    def run_all_benchmarks(self, iterations: int = 30) -> Dict[str, Any]:
        ...
        all_results["concurrent_execution"] = self.benchmark_concurrent_execution(iterations)
        
        print("Running workflow execution benchmark...")
        all_results["workflow_execution"] = self.benchmark_workflow_execution(iterations)
        ...
        all_results["resource_utilization"] = self.benchmark_resource_utilization(iterations)
        
        print("Running throughput scaling benchmark...")
        all_results["throughput_scaling"] = self.benchmark_throughput_scaling(iterations)
