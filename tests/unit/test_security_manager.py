...
class TestGitSafetyPolicy:
    ...
    def test_risk_level_assessment(self):
        ...
        # Critical risk
        assert (
            self.policy.get_risk_level("git push origin main", {}) == RiskLevel.CRITICAL
        )
        ...
        # Medium risk
        assert (
            self.policy.get_risk_level('git commit -m "test"', {}) == RiskLevel.MEDIUM
        )
...
class TestFileSystemSandbox:
    ...
    def test_blocks_access_to_forbidden_directories(self):
        ...
        forbidden_paths = ["/etc/passwd", "/usr/bin/python", "/root/.bashrc"]
    ...
    def test_denies_when_no_callback_configured(self):
        ...
        with pytest.warns(UserWarning):
            result = workflow.request_approval(
                "dangerous operation", RiskLevel.HIGH, {}
            )
...
class TestSecurityManager:
    ...
    def test_safe_network_request(self):
        ...
        allowed, reason = self.manager.safe_network_request(
            "https://api.anthropic.com/v1/messages"
        )
        assert allowed is True
    ...
    def test_audit_logging(self):
        ...
        self.manager.check_operation(
            "network", "request", {"url": "https://api.anthropic.com"}
        )
