...
class DependencyValidator:
    ...
    def _topological_sort(self) -> List[str]:
        ...
        if len(topo_order) != len(self.dependency_graph):
            ...
            raise ValueError(f"Topological sort failed - circular dependencies involving: {remaining}")

        return topo_order

    def get_parallel_execution_groups(self, result: DependencyValidationResult) -> List[List[str]]:
    ...
    def generate_dependency_report(self, result: DependencyValidationResult) -> str:
        ...
        if result.topological_order:
            lines.extend([
                "📊 Execution Order:",
                f"  {' → '.join(result.topological_order)}",
                ""
            ])
        ...
        if result.file_flows:
            ...
            for flow in result.file_flows:
                lines.append(f"  {flow.source_ticket} → {flow.target_ticket}: {flow.file_path}")
        ...
        if groups:
            ...
            for i, group in enumerate(groups, 1):
                if len(group) > 1:
                    lines.append(f"  Group {i}: {', '.join(group)} (can run in parallel)")
                else:
