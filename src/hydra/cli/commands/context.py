...
def add_context_parser(subparsers):
    ...
    context_parser = subparsers.add_parser(
        "context",
        help="Manage ticket execution context and artifacts"
    )
    ...
    context_subparsers.add_parser(
        "show",
        help="Show all tracked ticket contexts"
    )

    # Inspect specific ticket context
    inspect_parser = context_subparsers.add_parser(
        "inspect",
        help="Inspect detailed context for a specific ticket"
    )
    inspect_parser.add_argument(
        "ticket_id",
        help="Ticket ID to inspect"
    )

    # Clear context
    clear_parser = context_subparsers.add_parser(
        "clear",
        help="Clear context for a specific ticket or all tickets"
    )
    clear_parser.add_argument(
        "ticket_id",
        nargs="?",
        help="Ticket ID to clear (optional)"
    )
    clear_parser.add_argument(
        "--all",
        action="store_true",
        help="Clear all contexts"
    )

    # Show dependency context
    deps_parser = context_subparsers.add_parser(
        "deps",
        help="Show context that would be provided to a ticket"
    )
    deps_parser.add_argument(
        "ticket_id",
        help="Ticket ID to show dependencies for"
    )

    # Export contexts
    export_parser = context_subparsers.add_parser(
        "export",
        help="Export all ticket contexts"
    )
    ...
    preview_parser = context_subparsers.add_parser(
        "preview-updates",
        help="Preview how future tickets would be updated"
    )
    preview_parser.add_argument(
        "ticket_id",
        help="Completed ticket ID to preview updates for"
    )
