...
from hydra.parallel.async_executor import AsyncParallelExecutor, ExecutionStatus, TicketNode
from hydra.ticket_workflow import parse_ticket, mark_ticket_in_progress, mark_ticket_completed
...
class BatchExecutor(AsyncParallelExecutor):
    ...
    async def _analyze_and_create_batches(self, tickets_path: str, tickets: Dict[str, TicketNode]):
        ...
        for batch_id, batch in self.batches.items():
            logger.info(f"   {batch_id}: {len(batch.ticket_ids)} tickets ({batch.model})")
            
    def _create_batches_for_model(self, ticket_list: List[Tuple[str, dict]], model: str) -> List[BatchGroup]:
        ...
        while len(remaining_tickets) >= self.batch_config.min_tickets_for_batch:
            ...
            while (i < len(remaining_tickets) and 
                ...
                if current_complexity + ticket_complexity > self.batch_config.max_complexity_score:
                    can_add = False
                    
                # Check compatibility with all tickets in current batch
                if can_add:
                    for _, batch_ticket_data in current_batch:
                        if not self.analyzer.are_tickets_compatible(ticket_data, batch_ticket_data):
                            can_add = False
                            break
