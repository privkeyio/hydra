...
class TicketGenerator:
    ...
    def generate_tickets_yaml(
        self,
        project_description: str,
        output_path: str = "tickets.yaml",
        ...
        print(
            f"📊 Complexity analysis: {complexity['recommended_tickets']} ticket(s) - {complexity['rationale']}"
        )

        prompt = self._build_prompt(
            project_description, context, project_type, complexity
        )
        ...
        try:
            ...
            if "claude_tmux" in str(type(provider)):
                debug_path = (
                    Path(output_path).parent / ".hydra" / "last_claude_response.txt"
                )
    ...
    def _validate_and_fix_tickets(
        self, data: Dict[str, Any], project_description: str
    ) -> None:
        ...
        for ticket in tickets:
            ...
            if not is_valid:
                print(
                    f"⚠️  Fixing ticket {ticket.get('id', 'unknown')}: {', '.join(issues)}"
                )
    ...
    def _sync_to_dashboard(self, data: Dict[str, Any], project_dir: str) -> None:
        ...
        try:
            ...
            if os.path.exists(
                os.path.join(project_dir, "tickets.yaml")
            ) or os.path.exists(os.path.join(project_dir, "tickets.md")):
                os.environ["DATABASE_URL"] = (
                    f"sqlite:///{project_dir}/.hydra/dashboard/hydra.db"
                )
            ...
            with db_manager.get_session() as db:
                ...
                for ticket_data in data["tickets"]:
                    ...
                    existing = (
                        db.query(Ticket)
                        .filter_by(ticket_number=ticket_id, project_id=project.id)
                        .first()
                    )
                ...
                db.commit()
                print(
                    f"📊 Synced {tickets_added} new tickets to dashboard (total: {len(data['tickets'])} in file)"
                )
    ...
    def _parse_response(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                raise ValueError(
                    f"Expected dict with 'tickets' key or list of tickets, got {type(data)}"
                )
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("- id:") or (line.startswith("id:") and i == 0):
                ...
                if line.startswith("title:"):
                    ...
                    current_field == "description" and line and not line.startswith("-")
                ):
