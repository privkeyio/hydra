...
class PromptDetector:
    ...
    def _load_builtin_patterns(self):
        ...
        builtin_patterns = {
            'claude_permission_request': {
                ...
                'pattern': r'Do you want me to proceed\?|Should I continue\?|Would you like me to.*\?',
                'match_type': 'regex',
            ...
            'git_confirmation': {
                ...
                'tags': ['git', 'dangerous'],
                'metadata': {'tool': 'git', 'category': 'version_control', 'danger_level': 'high'}
            },
            'tmux_session_prompt': {
    ...
    def detect_prompt(self, text: str, context: Optional[Dict[str, Any]] = None) -> DetectionResult:
    ...
    def _match_pattern(self, pattern: PromptPattern, text: str) -> DetectionResult:
        ...
        if result.matched:
            result.suggested_response = self._get_suggested_response(pattern, result.match_groups)
            result.requires_user_input = pattern.response_strategy == ResponseStrategy.PROMPT_USER
        
        return result
    
    def _get_suggested_response(self, pattern: PromptPattern, match_groups: List[str]) -> Optional[str]:
    ...
    def _is_safety_blocked(self, pattern: PromptPattern, context: Optional[Dict[str, Any]]) -> bool:
        ...
        if self.config_manager:
            blocked_operations = self.config_manager.get('security.blocked_operations', [])
    ...
    def _capture_unknown_prompt(self, text: str, context: Optional[Dict[str, Any]]):
        ...
        Args:
            ...
            context: Optional context information
        """
        if text not in self._unknown_prompts and len(text.strip()) > 5:
    ...
    def generate_pattern_suggestions(self, min_frequency: int = 3) -> List[Dict[str, Any]]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self._stats['total_detections'] > 0:
            success_rate = self._stats['successful_matches'] / self._stats['total_detections']
...
class PromptHandler:
    ...
    def _handle_unknown_prompt(self, text: str, context: Optional[Dict[str, Any]]) -> str:
    ...
    def _prompt_user(self, text: str, result: Optional[DetectionResult]) -> str:
        ...
        if result and result.pattern:
            print(f"Pattern: {result.pattern.name} (confidence: {result.confidence:.2f})")
