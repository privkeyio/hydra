...
class TicketValidator:
    ...
    def validate_ticket(self, ticket: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if not criteria:
            ...
            if not file_creation_found and self._requires_file_creation(ticket):
                issues.append(
                    "No explicit file creation criteria (should start with 'Create [filename]')"
                )
    ...
    def transform_criteria_to_actionable(
        self, criteria: List[str], project_type: str = None
    ) -> List[str]:
    ...
    def _transform_single_criterion(
        self, criterion: str, project_type: str = None
    ) -> str:
        ...
        if "calculator" in criterion_lower or project_type == "calculator":
            if "display" in criterion_lower and (
                "page" in criterion_lower or "web" in criterion_lower
            ):
        ...
        if not any(
            criterion_lower.startswith(v)
            for v in ["create", "implement", "add", "test", "verify"]
        ):
