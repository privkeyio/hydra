...
class TicketUpdater:
    ...
    def update_future_tickets(
        self, completed_ticket_id: str, artifacts: List[TicketArtifact]
    ) -> int:
        ...
        if dependent_tickets:
            print(
                f"🔄 Found {len(dependent_tickets)} tickets that depend on Ticket {completed_ticket_id}"
            )
            ...
            for dep_ticket_id in dependent_tickets:
                ...
                if ticket_updates:
                    ...
                    updates_made += 1
                    print(
                        f"   ✏️ Updated Ticket {dep_ticket_id} with specific file references"
                    )
        ...
        if content != original_content:
            with open(self.tickets_path, "w") as f:
                f.write(content)
            print(
                f"✅ Updated {updates_made} ticket sections with specific artifact references"
            )
    ...
    def _create_artifact_map(
        self, artifacts: List[TicketArtifact]
            ...
            if ref_type == "file_reference":
                ...
                for match in re.finditer(pattern, ticket_section, re.IGNORECASE):
                    ...
                    if ticket_num == referenced_ticket:
                        ...
                        if matching_artifacts:
                            ...
                            ticket_section = ticket_section.replace(
                ...
                for match in matches:
                    ...
                    if "- " in before_text and not any(
                        ...
                        all_artifacts = [
                            a for artifacts in artifact_map.values() for a in artifacts
                        ]
                        if all_artifacts:
                            ...
                            if len(all_artifacts) > 3:
                                file_list = (
                                    ", ".join(main_files)
                                    + f" and {len(all_artifacts)-3} more"
                                )
                            else:
                                file_list = ", ".join(main_files)
                            replacement = (
                                f"(from Ticket {referenced_ticket}: {file_list})"
                            )
                            ticket_section = ticket_section.replace(
                                match.group(0), replacement
                            )
    ...
    def _update_informal_references(
        self, content: str, completed_ticket: str, artifacts: List[TicketArtifact]
    ) -> Tuple[str, int]:
