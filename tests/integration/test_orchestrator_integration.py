...
"""
            ...
            "tasks": [
                {
                    ...
                    "complexity": "simple",
                    "parameters": {"sources": ["api", "database", "files"]},
                },
                {
                    "id": "validate_data",
                    "name": "Validate Data",
    ...
    def test_model_routing_logic(self, sample_yaml_project):
        ...
        simple_task = next(
            t for t in spec.tasks if t.complexity == TaskComplexity.SIMPLE
        )
        ...
        critical_task = next(
            t for t in spec.tasks if t.complexity == TaskComplexity.CRITICAL
        )
    ...
    async def test_parallel_execution_limits(self):
        ...
        for i in range(12):
            tasks.append(
                TaskNode(
                    id=f"task_{i}", name=f"Task {i}", description=f"Parallel task {i}"
                )
            )
    ...
    def test_execution_report_generation(self, sample_yaml_project):
        ...
        assert (
            report["task_results"]["integrate_frontend_backend"]["status"] == "failed"
        )
        assert (
            report["task_results"]["integrate_frontend_backend"]["error"]
            == "Test failure"
        )
