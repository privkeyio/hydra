...
class WorkStealingAsyncExecutor:
            ...
            if node.status == ExecutionStatus.PENDING:
                ...
                if not unresolved_deps:
                    ...
                    if self.work_stealing_scheduler.submit_task(task):
                        ready_count += 1
                        logger.debug(
                            f"Submitted ticket {ticket_id} to work stealing scheduler"
                        )
                    else:
    ...
    def _calculate_task_priority(self, node: TicketNode) -> int:
        ...
        dependency_count = sum(
            1
            for other_node in self.tickets.values()
    ...
    async def execute_ticket(
        self, ticket_id: str, worker_id: str, tickets_path: str
    ) -> bool:
        """Execute a single ticket asynchronously with work stealing integration."""
        start_delay = asyncio.create_task(
            asyncio.sleep(0.1 * hash(ticket_id) % 50 / 10)
        )
        await start_delay
...
REMINDER: You are working on Ticket {ticket_id} ONLY. Ignore all other tickets."""
            ...
            if result.status.value == "completed":
                ...
                self.work_stealing_scheduler.complete_task(
                    worker_id, task_obj, quality_passed
                )
        ...
        if completed_ticket_id in self.dependency_waiters:
            for waiting_ticket in self.dependency_waiters[completed_ticket_id]:
                ...
                if not unresolved_deps and waiting_ticket not in self.running_tickets:
                    ...
                    if self.work_stealing_scheduler.submit_task(task):
                        logger.debug(
                            f"Submitted dependent ticket {waiting_ticket} to work stealing scheduler"
                        )
    ...
    async def execute_with_work_stealing(self, tickets_path: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Stealing policy: {self.work_stealing_scheduler.stealing_policy.value}"
        )
        ...
        async def worker_coroutine(worker_id: str):
            ...
            while True:
                ...
                if task is None:
                    ...
                    pending_tasks = sum(
                        len(queue)
                        for queue in self.work_stealing_scheduler.worker_queues.values()
                    )
                ...
                # Execute the task
                success = await self.execute_ticket(
                    task.task_id, worker_id, tickets_path
                )
                logger.debug(
                    f"Worker {worker_id} completed task {task.task_id}: {success}"
                )
