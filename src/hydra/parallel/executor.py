...
from hydra.orchestrator.claude_code_orchestrator import ClaudeCodeOrchestrator, ClaudeCodeTask
from hydra.quality import QualityGateRunner
from hydra.ticket_workflow import parse_ticket, mark_ticket_completed
from hydra.verification import TicketVerifier
    ...
    def build_execution_plan(self) -> ExecutionPlan:
        ...
        while len(processed) < len(self.tickets):
            ...
            if not wave:
                ...
                print(f"⚠️  Warning: Circular or missing dependencies detected for: {remaining}")
...
"""
            ...
            if result.status.value == "completed":
                ...
                quality_passed = quality_report.overall_status.value in ["passed", "warning"]
    ...
    def execute_plan(self, plan: ExecutionPlan, tickets_path: str) -> Dict[str, Any]:
        ...
        for wave_num, wave in enumerate(plan.waves, 1):
            ...
            for ticket_id in wave_to_execute:
                node = self.tickets[ticket_id]
                deps_failed = any(dep in self.failed_tickets for dep in node.dependencies)
            ...
            if blocked_tickets:
                print(f"⛔ Blocked tickets due to failed dependencies: {', '.join(blocked_tickets)}")
        ...
        summary = {
            ...
            "blocked": sum(1 for n in self.tickets.values() if n.status == ExecutionStatus.BLOCKED),
            "duration": duration,
            "success_rate": len(self.completed_tickets) / plan.total_tickets * 100 if plan.total_tickets > 0 else 0,
            "results": all_results
        }
    ...
    def save_execution_log(self, summary: Dict[str, Any], log_dir: Optional[str] = None):
        """Save execution log."""
        if not log_dir:
