...
class TestTaskNode:
    ...
    def test_can_execute_with_satisfied_dependencies(self):
        task = TaskNode(
            id="task2", name="Task", description="desc", dependencies=["task1"]
        )
...
"""
    ...
    def test_validate_parallel_limits(self):
        spec = ProjectSpecification(
            name="Test", description="Test", tasks=[], max_parallel=11
        )
        valid, errors = spec.validate()
        assert valid is False
...
class TestProjectOrchestrator:
    ...
    def test_select_model_for_task_manual(self, mock_config):
        ...
        task = TaskNode(
            id="task1", name="Task", description="test", model=ModelType.OPUS
        )
    ...
    def test_analyze_task_complexity(self, mock_config):
        ...
        simple_task = TaskNode(
            id="t1", name="Task", description="create a list of items"
        )
        assert (
            orchestrator._analyze_task_complexity(simple_task) == TaskComplexity.SIMPLE
        )

        complex_task = TaskNode(
            id="t2", name="Task", description="architect the system"
        )
        assert (
            orchestrator._analyze_task_complexity(complex_task)
            == TaskComplexity.COMPLEX
        )

        critical_task = TaskNode(
            id="t3", name="Task", description="implement security measures"
        )
        assert (
            orchestrator._analyze_task_complexity(critical_task)
    ...
    @pytest.mark.skipif(
        TEST_MODE,
        reason="Skip timeout tests - synchronous execution in test mode doesn't timeout",
    )
