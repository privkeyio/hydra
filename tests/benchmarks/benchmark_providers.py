...
class ProviderBenchmark:
        ...
        for name, provider in providers.items():
            ...
            def generate_json():
                return provider.generate_json(
                    "Create a JSON response with plan and code"
                )
                ...
                with ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    for i in range(10):
                        future = executor.submit(
                            provider.generate, f"Generate function #{i}"
                        )
    ...
    def benchmark_memory_usage(self, iterations: int = 50) -> Dict[str, Any]:
        ...
        result1 = self.runner.run_benchmark(
            name="memory_intensive", func=memory_intensive, iterations=iterations
        )
        results["intensive"] = result1

        result2 = self.runner.run_benchmark(
            name="memory_efficient", func=memory_efficient, iterations=iterations
        )
    ...
    def benchmark_session_management(self, iterations: int = 50) -> Dict[str, Any]:
        ...
        result1 = self.runner.run_benchmark(
            name="session_creation", func=create_session, iterations=iterations
        )
    ...
    def run_all_benchmarks(self, iterations: int = 50) -> Dict[str, Any]:
        ...
        all_results["concurrent_requests"] = self.benchmark_concurrent_requests(
            iterations
        )
        ...
        all_results["session_management"] = self.benchmark_session_management(
            iterations
        )
