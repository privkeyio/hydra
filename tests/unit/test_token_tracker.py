...
class TestTokenTracker:
    ...
    def test_check_budget_available(self, mock_db_manager, mock_cache):
        ...
        if not available:
            ...
            print(f"DEBUG: Calculated cost: {cost}, Budget: {tracker.budget_limit}, Usage: {tracker._current_usage.total_cost}")
        assert available == True, f"Expected budget to be available. Message: {message}, Type: {type(available)}"
