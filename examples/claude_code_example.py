...
def claude_code_session():
    ...
    # Task 4: Run tests
    print("\n4. Running tests...")
    test_result = agent.execute_command("python -m pytest tests/ -v")
...
def stateful_project_management():
    ...
    state_manager = ProjectStateManager(
        project_id="claude_project_001", storage_path="./project_states"
    )
    ...
    try:
        ...
        for idx, task in enumerate(tasks[start_idx:], start=start_idx):
            ...
            state_manager.add_operation(
                {"task": task, "index": idx, "status": "in_progress"}
            )

            # Execute task
            result = agent.execute_task(
                {"type": "code_generation", "description": task}
            )

            # Record completion
            state_manager.add_operation(
                {"task": task, "index": idx, "status": "completed", "result": result}
            )

            # Create checkpoint every 3 tasks
            if (idx + 1) % 3 == 0:
                checkpoint_id = state_manager.create_checkpoint(
                    f"Checkpoint after {task}", metadata={"task_index": idx}
                )
...
def monitoring_dashboard_demo():
    ...
    dashboard_thread = threading.Thread(
        target=start_dashboard, kwargs={"host": "127.0.0.1", "port": 8001}, daemon=True
    )
    ...
    agents = [ClaudeCodeAgent(f"agent_{i}") for i in range(3)]
    ...
    monitoring.record_workflow_execution(
        workflow_id="demo_workflow", duration=15.5, tasks_completed=12, tasks_failed=3
    )
