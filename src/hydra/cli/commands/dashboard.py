...
def add_dashboard_parser(subparsers):
    ...
    dashboard_parser = subparsers.add_parser(
        "dashboard", help="Manage the Hydra web dashboard"
    )
    ...
    dashboard_parser.add_argument(
        "--port", type=int, default=8080, help="Port for dashboard (default: 8080)"
    )
    dashboard_parser.add_argument(
...
def handle_dashboard_command(args) -> int:
    ...
    cmd = [python_exe, "-m", "hydra.dashboard.standalone", action]
