...
from hydra.templates.template_engine import TemplateEngine, Template, TemplateParameter
...
class TestTemplateEngine:
    ...
    def test_resolve_parameters_success(self):
        ...
        template_params = [
            ...
            TemplateParameter("optional_param", "string", "Optional", default="default", required=False),
            TemplateParameter("choice_param", "string", "Choice", choices=["opt1", "opt2"], default="opt1")
        ]
    ...
    def test_resolve_parameters_invalid_choice(self):
        engine = TemplateEngine()
        template_params = [
            TemplateParameter("choice_param", "string", "Choice", choices=["opt1", "opt2"])
        ]
        
        user_params = {"choice_param": "invalid"}
        
        with pytest.raises(ValueError, match="Invalid choice for choice_param: invalid"):
            engine._resolve_parameters(template_params, user_params)
    ...
    def test_generate_project_success(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = engine.generate_project("test_template", output_dir, {"project_name": "MyProject"})
            
            assert result["template"] == "test_template"
            assert result["output_dir"] == str(output_dir)
            assert result["parameters"]["project_name"] == "MyProject"
            assert len(result["generated_files"]) == 2
            assert result["post_generation_commands"] == ["echo 'Project {{project_name}} created'"]
...
class TestTemplateValidator:
    ...
    def test_validate_template_structure_missing_dir(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            is_valid, errors = TemplateValidator.validate_template_structure(nonexistent_dir)
    ...
    def test_validate_template_structure_missing_required_fields(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert not is_valid
            assert any("Missing required field" in error and "name" in error for error in errors)
            assert any("Missing required field" in error and "description" in error for error in errors)
    ...
    def test_generate_test_parameters(self):
        parameters = [
            ...
            TemplateParameter("choice_param", "string", "Choice param", choices=["opt1", "opt2"]),
            TemplateParameter("default_param", "string", "Default param", default="default_value")
        ]
