...
from hydra.providers.interactive_base import ProviderCapability
...
class TestProviderComposition:
    ...
    def test_successful_composition(self):
        ...
        task_req = TaskRequirement(required_capabilities=missing, minimum_providers=2)
...
class TestCapabilityManager:
    ...
    def setup_method(self):
        ...
        self.provider1 = MockProvider(
            "provider1",
            {ProviderCapability.FILE_OPERATIONS, ProviderCapability.SHELL_EXECUTION},
        )
        self.provider2 = MockProvider(
            "provider2",
            {
    ...
    def test_cache_serialization(self):
        ...
        assert cache_entry.capabilities == {
            ProviderCapability.FILE_OPERATIONS,
            ProviderCapability.SHELL_EXECUTION,
        }
...
class TestIntegrationScenarios:
    ...
    def setup_method(self):
        ...
        self.file_provider = MockProvider(
            "file_provider",
            {
                ProviderCapability.FILE_OPERATIONS,
                ProviderCapability.SESSION_PERSISTENCE,
        ...
        self.shell_provider = MockProvider(
            "shell_provider",
            {ProviderCapability.SHELL_EXECUTION, ProviderCapability.TASK_EXECUTION},
        )

        self.streaming_provider = MockProvider(
            "streaming_provider",
            {ProviderCapability.STREAMING_RESPONSE, ProviderCapability.PROMPT_HANDLING},
        )

        self.full_featured_provider = MockProvider(
            "full_provider",
            {
                ProviderCapability.FILE_OPERATIONS,
                ProviderCapability.SHELL_EXECUTION,
