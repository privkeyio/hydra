...
class WorkStealingScheduler:
        ...
        with self.global_lock:
            ...
            if task.assigned_at:
                ...
                if total_tasks > 0:
                    ...
                    speed_factor = min(1.0, 60.0 / max(metrics.average_task_time, 1.0))  # Assume 60s is baseline
    ...
    def _find_steal_candidates(self, stealing_worker_id: str) -> List[str]:
        ...
        for worker_id, metrics in self.worker_metrics.items():
            ...
            stealing_load = stealing_metrics.active_tasks + len(self.worker_queues[stealing_worker_id])
    ...
    def _rebalance_loop(self):
        """Main rebalancing loop that runs periodically."""
    ...
    def _move_tasks(self, from_worker: str, to_workers: List[str]) -> int:
        ...
        moved_count = 0
        max_moves = max(1, len(self.worker_queues[from_worker]) // 3)
        ...
        with self.worker_locks[from_worker]:
            ...
            while moved_count < max_moves and len(from_queue) > 1:
                task_to_move = from_queue.pop()  # Take from end
                target_worker = min(to_workers, key=lambda w: len(self.worker_queues[w]))
