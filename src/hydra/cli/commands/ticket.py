...
def add_ticket_parser(subparsers):
    ...
    ticket_subparsers = ticket_parser.add_subparsers(
        dest="ticket_action", help="Ticket operations"
    )

    # Create tickets command
    create_tickets_parser = ticket_subparsers.add_parser(
        "create", help="Generate tickets.md file from project description"
    )
    create_tickets_parser.add_argument(
        "description", help="Project description to generate tickets from"
    )
    create_tickets_parser.add_argument(
        "--output",
        "-o",
    ...
    execute_ticket_parser = ticket_subparsers.add_parser(
        "execute", help="Execute a single ticket by identifier"
    )
    execute_ticket_parser.add_argument("tickets", help="Path to tickets.md file")
    execute_ticket_parser.add_argument(
        "identifier", help="Ticket identifier to execute (e.g., 001, 002)"
    )
    execute_ticket_parser.add_argument(
        "--skip-preflight",
        action="store_true",
    ...
    run_tickets_parser = ticket_subparsers.add_parser(
        "run-all", help="Execute all tickets in order with dependency resolution"
    )
    run_tickets_parser.add_argument("tickets", help="Path to tickets.md file")
    ...
    auto_parser = ticket_subparsers.add_parser(
        "auto", help="Automated workflow with dependency-aware parallel execution"
    )
    auto_parser.add_argument("tickets", help="Path to tickets.md file")
    ...
    verify_parallel_parser.add_argument(
        "tickets", help="Path to tickets.md file to verify against"
    )
    verify_parallel_parser.add_argument(
        "--workers",
        "-w",
...
def _handle_run_all_tickets(args) -> int:
    ...
    try:
        ...
        success = run_all_tickets(
            args.tickets, skip_preflight=skip_preflight, max_parallel=max_parallel
        )
        return 0 if success else 1
    except Exception as e:
...
def _handle_auto_workflow(args) -> int:
    ...
    try:
        ...
        skip_preflight = getattr(args, "skip_preflight", False)
        success = run_all_tickets(
            str(tickets_path), max_parallel=args.parallel, skip_preflight=skip_preflight
        )
...
def _handle_verify_parallel(args) -> int:
    ...
    try:
        ...
        print("🤖 Agents will verify and complete any unfinished acceptance criteria")
        ...
        def verify_ticket_with_agent(ticket_id: str) -> bool:
            ...
            print(
                f"⏱️  Ticket {ticket_id} verification starting in {start_delay:.1f}s..."
            )
...
Report what you found and what you completed."""
                ...
                if result.status.value == "completed":
                    ...
                    if validation_passed:
                        ...
                        print(
                            f"❌ Ticket {ticket_id} - agent ran but criteria still not met"
                        )
                        return False
                else:
        ...
        if args.save_report:
            ...
            report_path = (
                Path(".hydra/reports")
                / f"verify_parallel_{Path(tickets_path).stem}.json"
            )
