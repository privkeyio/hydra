...
class TestProviderDiscovery:
    ...
    def test_discover_all(self, mock_is_available):
        ...
        claude_config = next(
            (c for c in discovered if c.provider_name == "claude_cli"), None
        )
    ...
    def test_probe_tool_capabilities(self, mock_run):
        ...
        capabilities = ProviderDiscovery.probe_tool_capabilities(
            "/usr/bin/timeout-tool"
        )
        assert len(capabilities) == 0
    ...
    def test_create_config_from_path(self):
        ...
        with patch.object(ProviderDiscovery, "probe_tool_capabilities") as mock_probe:
            ...
            config = ProviderDiscovery.create_config_from_path(
                "/usr/bin/tool", "custom_name"
            )
    ...
    def test_scan_directory(self):
        ...
        with (
            ...
            claude_config = next(
                (c for c in configs if c.provider_name == "claude_cli"), None
            )
            assert claude_config is not None
    ...
    def test_discover_all_no_tools_available(self):
        ...
        with patch.object(
            ProviderDiscovery, "_is_command_available", return_value=False
        ):
            discovered = ProviderDiscovery.discover_all()
