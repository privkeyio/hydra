...
class TestWorkStealingPerformance(unittest.TestCase):
    ...
    def process_tasks_no_stealing(self, num_workers: int, tasks: List[Task]) -> float:
        ...
        for i in range(num_workers):
            thread = threading.Thread(target=worker_thread, args=(i, worker_queues[i]))
    ...
    def process_tasks_with_stealing(
        self, scheduler: WorkStealingScheduler, tasks: List[Task]
    ) -> float:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(scheduler.worker_queues)
        ) as executor:
            futures = []
    ...
    def test_performance_improvement(self):
        ...
        try:
            ...
            print(
                f"Run {run + 1}: Baseline={baseline_time:.2f}s, "
            ...
            total_stolen = sum(
                m["tasks_stolen_to"] for m in metrics["worker_metrics"].values()
            )
            self.assertGreater(
                total_stolen, 0, "No work stealing occurred during performance test"
            )
    ...
    def test_rebalancing_performance(self):
        ...
        try:
            ...
            for i in range(num_tasks):
                task_with = Task(task_id=f"rebalance_task_{i}", estimated_duration=0.02)
                task_without = Task(
                    task_id=f"no_rebalance_task_{i}", estimated_duration=0.02
                )
                ...
                with scheduler_no_rebalance.worker_locks["worker_0"]:
                    scheduler_no_rebalance.worker_queues["worker_0"].append(
                        task_without
                    )
            ...
            time_with = self.process_tasks_with_stealing(
                scheduler_with_rebalance, tasks_with
            )
            ...
            time_without = self.process_tasks_with_stealing(
                scheduler_no_rebalance, tasks_without
            )
            ...
            print(
                f"Rebalancing test: with={time_with:.2f}s, without={time_without:.2f}s"
            )
            print(
                f"Rebalancing improvement: {((time_without - time_with) / time_without * 100):.1f}%"
            )
        except RuntimeError as e:
            if "can't start new thread" in str(e):
                self.skipTest("Skipping test in CI environment - thread limit reached")
    ...
    def test_stealing_policy_performance(self):
        ...
        try:
            ...
            for policy in policies_to_test:
                ...
                print(
                    f"Policy {policy.value}: time={execution_time:.2f}s, "
