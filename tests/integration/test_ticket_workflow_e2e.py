...
def run_subprocess_safe(cmd, env=None, timeout=60, cwd=None):
    ...
    try:
        ...
        return subprocess.CompletedProcess(
            cmd, 1, stdout=f"Process timed out after {timeout} seconds", stderr=str(e)
        )
    except OSError as e:
        ...
        if "Resource temporarily unavailable" in str(e):
            ...
            try:
                ...
                return subprocess.CompletedProcess(
                    cmd, 1, stdout="", stderr=f"Resource exhaustion: {retry_error}"
                )
        return subprocess.CompletedProcess(cmd, 1, stdout="", stderr=f"OS Error: {e}")
...
def hydra_cli():
    ...
    try:
        result = run_subprocess_safe(["which", "hydra"], timeout=5)
...
"""
        ...
        assert (
            "All tickets completed successfully" in result.stdout
            or "All tickets executed successfully" in result.stdout
        )
...
"""
        ...
        assert (
            f"Max parallel workers: {workers}" in result.stdout
            or f"workers: {workers}" in result.stdout.lower()
        )
