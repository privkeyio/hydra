...
from hydra.providers.base import LLMProvider, LLMConfig
...
class MockLLMProvider(LLMProvider):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        if "error" in prompt_lower or "fail" in prompt_lower:
            ...
            return json.dumps({"mock": "json_response", "prompt_hash": hash(prompt) % 1000})
        else:
    ...
    def generate_json(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        self.call_count += 1
        self.call_history.append({"type": "generate_json", "prompt": prompt, "kwargs": kwargs})
...
class MockAnthropicProvider(MockLLMProvider):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        if self.safety_filters and ("unsafe" in prompt.lower() or "dangerous" in prompt.lower()):
...
def create_mock_provider(provider_type: str, config: Optional[LLMConfig] = None) -> MockLLMProvider:
    """Factory function to create mock providers."""
    if config is None:
        config = LLMConfig(provider_type=provider_type, model=f"{provider_type}-test-model")
