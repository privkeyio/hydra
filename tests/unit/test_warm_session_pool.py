...
from hydra.providers.warm_session_pool import WarmSessionPool
...
class MockProvider(InteractiveAIProvider):
    ...
    def execute_task(
        self, session_id: str, task_prompt: str, timeout=None
    ) -> TaskResult:
        if session_id not in self.started_sessions:
            raise KeyError(f"Session {session_id} not found")

        return TaskResult(
            task_id="test_task", success=True, output="Task completed successfully"
        )
...
class TestWarmSessionPool:
    ...
    def test_pool_initialization(self):
        ...
        with WarmSessionPool(
            self.provider_factory, min_pool_size=2, max_pool_size=4
        ) as pool:
    ...
    def test_session_acquisition_speed(self):
        ...
        with WarmSessionPool(
            self.provider_factory, min_pool_size=3, max_pool_size=5
        ) as pool:
    ...
    def test_session_health_checking(self):
        ...
        with WarmSessionPool(failing_factory, min_pool_size=1, max_pool_size=2) as pool:
            time.sleep(0.1)
    ...
    def test_dynamic_pool_sizing(self):
        ...
        with WarmSessionPool(
            self.provider_factory, min_pool_size=2, max_pool_size=5
        ) as pool:
            time.sleep(0.1)
    ...
    def test_pool_shutdown(self):
        ...
        pool = WarmSessionPool(self.provider_factory, min_pool_size=2, max_pool_size=4)
    ...
    def test_provider_factory_failure_handling(self):
        ...
        with WarmSessionPool(failing_factory, min_pool_size=2, max_pool_size=4) as pool:
    ...
    @pytest.mark.parametrize("min_size,max_size", [(1, 2), (3, 5), (2, 10)])
    def test_different_pool_sizes(self, min_size, max_size):
        """Test pool works with different size configurations."""
        with WarmSessionPool(
            self.provider_factory, min_pool_size=min_size, max_pool_size=max_size
        ) as pool:
            time.sleep(0.1)
