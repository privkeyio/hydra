...
from hydra.models.db import get_db_manager
from hydra.performance import get_optimized_session, pool_manager
...
async def test_database_connection_pooling(test_db):
    ...
    assert (
        metrics.get_avg_latency() < 100
    ), f"Average query time {metrics.get_avg_latency()}ms exceeds 100ms"
    assert (
        metrics.errors == 0
    ), f"Got {metrics.errors} errors during connection pool test"
    assert (
        metrics.get_throughput() > 50
    ), f"Throughput {metrics.get_throughput()} req/s is below 50 req/s"
...
async def test_http_connection_pooling():
    ...
    assert (
        metrics.get_avg_latency() < 500
    ), f"Average HTTP request time {metrics.get_avg_latency()}ms exceeds 500ms"
...
async def test_concurrent_request_handling():
    ...
    async def simulate_request(request_id: int):
        ...
        try:
            ...
            async with get_optimized_session() as session:
                # Read operation
                result = await session.execute(text("SELECT id FROM tasks LIMIT 1"))
                _ = result.scalar()
    ...
    assert (
        metrics.get_p95_latency() < 2000
    ), f"P95 latency {metrics.get_p95_latency()}ms exceeds 2000ms"
    assert (
        len(metrics.latencies) >= 100
    ), f"Only handled {len(metrics.latencies)} requests, expected 100+"
