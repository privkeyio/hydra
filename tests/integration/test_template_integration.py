...
"""
    ...
    def _create_cli_tool_template(self):
        ...
        config = {
            ...
            "post_generation_commands": ["chmod +x {{tool_name}}"],
            "tags": ["cli", "tool"],
        }
...
"""
    ...
    def test_load_valid_web_app_template(self):
        ...
        assert (
            len(template.files) == 5
        )  # app.py, package.json, requirements.txt, style.css, config file
    ...
    def test_generate_cli_tool_project(self):
        ...
        parameters = {"tool_name": "mytool", "language": "python"}
    ...
    def test_parameter_validation_edge_cases(self):
        ...
        with pytest.raises(ValueError, match="Required parameter missing: app_name"):
            self.engine.generate_project(
                "web_app", output_dir, {"database": "postgresql"}
            )

        # Test invalid choice
        with pytest.raises(ValueError, match="Invalid choice for database: oracle"):
            self.engine.generate_project(
                "web_app", output_dir, {"app_name": "TestApp", "database": "oracle"}
            )
