...
def parse_ticket_compat(
    tickets_path: str, ticket_identifier: str
    ...
    if _should_use_database(tickets_path):
        try:
            ...
            print(
                f"Database access failed, falling back to markdown: {e}",
                file=sys.stderr,
            )
    ...
    if _should_use_database(tickets_path):
        try:
            ...
            service.update_ticket_status(
                project_name, ticket_identifier, "DONE", update_criteria=True
            )
        except Exception:
...
def mark_ticket_quality_failed_compat(
    tickets_path: str, ticket_identifier: str, quality_report: Any = None
) -> bool:
    ...
    if _should_use_database(tickets_path):
        try:
            ...
            # Update status in database
            service.update_ticket_status(
                project_name, ticket_identifier, "QUALITY_FAILED"
            )
        except Exception:
            pass  # Database update is optional
...
def add_ticket_artifact_compat(
    tickets_path: str,
...
def build_dependency_graph_compat(
